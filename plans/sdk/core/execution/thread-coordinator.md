# thread-coordinator.ts - Thread协调器逻辑设计

## 需求分析

Thread协调器需要提供以下核心能力：
1. 创建子Thread
2. 协调子Thread的执行（串行/并行）
3. 等待子Thread完成
4. 根据策略合并子Thread的结果
5. 处理超时和错误

参考fork-join-manager-logic.md的设计，将ForkManager、JoinManager和ForkJoinCoordinator合并为一个简化的ThreadCoordinator。
...
- stateManager: Thread状态管理器，用于创建和管理Thread
- workflowExecutor: 工作流执行器，用于执行子工作流
...
接收父线程ID和Fork配置，创建子Thread并返回子Thread数组。
...
### 2. executeThreads方法 - 执行子Thread

接收子Thread数组和执行策略，执行子Thread。

执行步骤：

如果forkStrategy为SERIAL：
- 调用executeSerialThreads方法
- 串行执行子Thread

如果forkStrategy为PARALLEL：
- 调用executeParallelThreads方法
- 并行执行子Thread

### 3. executeSerialThreads方法 - 串行执行子Thread
...
步骤2：依次执行子Thread
- 对每个子Thread：
  - 调用WorkflowExecutor的execute方法
  - 传入子Thread对应的工作流定义和选项
  - 等待执行完成
  - 将结果添加到results数组
...
### 4. executeParallelThreads方法 - 并行执行子Thread
...
步骤2：并行执行子Thread
- 对每个子Thread：
  - 创建Promise，调用WorkflowExecutor的execute方法
...
### 5. join方法 - 合并子Thread结果

接收子Thread数组和Join配置，等待子Thread完成并合并结果。
...
### 6. waitForCompletion方法 - 等待子Thread完成
