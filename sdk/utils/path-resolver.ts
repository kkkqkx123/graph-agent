 ...
 *
 * 使用示例：
 * - resolve("user.name", obj) - 获取嵌套属性值
 * - resolve("items[0].name", obj) - 获取数组元素属性
 * - set("user.name", obj, "John") - 设置嵌套属性值
 * - exists("user.name", obj) - 检查路径是否存在
 */
...
export class PathResolver {
   ...
   */
  static resolve(path: string, root: any): any {
    if (!path || !root) {
      return undefined;
    }
   ...
   * @returns 路径是否存在
   */
  static exists(path: string, root: any): boolean {
    try {
      return this.resolve(path, root) !== undefined;
    } catch {
      return false;
    }
  }
   ...
   * @returns 是否设置成功
   */
  static set(path: string, root: any, value: any): boolean {
    if (!path || !root) {
      return false;
    }
