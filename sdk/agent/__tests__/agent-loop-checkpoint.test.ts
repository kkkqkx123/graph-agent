...
import type { AgentLoopConfig, AgentLoopCheckpoint, CheckpointMetadata } from '@modular-agent/types';
...
describe('Agent Loop Checkpoint', () => {
  ...
  describe('Delta Checkpoint Creation', () => {
    ...
    it('should reference previous checkpoint', async () => {
      ...
      const firstId = await AgentLoopCheckpointCoordinator.createCheckpoint(entity, mockDependencies);

      entity.addMessage({ role: 'user', content: 'New' });

      const secondId = await AgentLoopCheckpointCoordinator.createCheckpoint(entity, mockDependencies);
  ...
  describe('Checkpoint Type Decision', () => {
    ...
    it('should always create full checkpoint when delta disabled', async () => {
      ...
      entity.addMessage({ role: 'user', content: 'New' });
      const secondId = await AgentLoopCheckpointCoordinator.createCheckpoint(entity, disabledDependencies);
