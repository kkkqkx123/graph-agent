...
import type { MessageHistory } from '../../../core/messages/message-history.js';
 ...
 */
export type AgentLoopStreamEvent = MessageStreamEvent | {
...
export class AgentStreamExecutor {
    ...
    async *execute(
        ...
        yield {
            ...
            timestamp: Date.now(),
            data: { config: entity.config, loopId: agentLoopId }
        };
    ...
    private async *executeIterationStream(
        ...
        yield* this.executeToolCallsStream(entity, messageHistory, finalResult.toolCalls);
    ...
    private async *executeToolCallsStream(
        ...
        for (const result of toolResults) {
            ...
            yield {
                ...
                timestamp: Date.now(),
                data: { toolCall: { id: result.toolCallId, name: toolCallInfo.name, arguments: args } }
            };
            ...
            if (result.success) {
                ...
                yield {
                    ...
                    timestamp: Date.now(),
                    data: { toolCallId: result.toolCallId, result: result.result, success: true }
                };
            } else {
                ...
                yield {
                    type: AgentStreamEventType.TOOL_CALL_END,
                    timestamp: Date.now(),
                    data: { toolCallId: result.toolCallId, error: result.error, success: false }
                };
            }
        }
    }
