 ...
 * @returns 验证结果
 */
export function validateContextProcessorNode(node: Node): Result<Node, ValidationError[]> {
  if (node.type !== NodeType.CONTEXT_PROCESSOR) {
    return err([new ValidationError(`Invalid node type for context processor validator: ${node.type}`, `node.${node.id}`)]);
  }
  ...
  if (!result.success) {
    ...
    if (!error) {
      return err([new ValidationError('Invalid context processor node configuration', `node.${node.id}.config`)]);
    }
