 ...
 * @returns 验证结果
 */
export function validateForkNode(node: Node): Result<Node, ValidationError[]> {
  if (node.type !== NodeType.FORK) {
    return err([new ValidationError(`Invalid node type for fork validator: ${node.type}`, `node.${node.id}`)]);
  }
  ...
  if (!result.success) {
    ...
    if (!error) {
      return err([new ValidationError('Invalid fork node configuration', `node.${node.id}.config`)]);
    }
