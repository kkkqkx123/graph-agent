...
describe('MessageValidator', () => {
  ...
  describe('validateRole', () => {
    it('should validate valid roles', () => {
      ...
      validRoles.forEach(role => {
        ...
        expect(result.isOk()).toBe(true);
        expect(result.unwrap()).toBe(role);
      });
    });
    ...
    it('should return error for invalid role', () => {
      ...
      if (result.isErr()) {
        expect(result.error.length).toBeGreaterThan(0);
        expect(result.error[0]?.message).toContain('Invalid message role');
      }
    });
  });
  ...
  describe('validateContent', () => {
    ...
    it('should validate valid array content', () => {
      ...
      expect(result.isOk()).toBe(true);
      expect(result.unwrap()).toEqual(content);
    });
  ...
  describe('validateToolCalls', () => {
    ...
    it('should validate valid tool calls', () => {
      ...
      expect(result.isOk()).toBe(true);
      expect(result.unwrap()).toEqual(toolCalls);
    });
  ...
  describe('validateToolCallId', () => {
    it('should validate valid tool call id', () => {
      ...
      expect(result.isOk()).toBe(true);
      expect(result.unwrap()).toBe('call_123');
    });
  ...
  describe('validateMessages', () => {
    it('should validate valid messages array', () => {
      ...
      expect(result.isOk()).toBe(true);
      expect(result.unwrap()).toEqual(messages);
    });
    ...
    it('should include path prefix for errors in array', () => {
      ...
      expect(result.isErr()).toBe(true);
      if (result.isErr()) {
        expect(result.error.some(e => e.field?.includes('messages[1]'))).toBe(true);
      }
    });
