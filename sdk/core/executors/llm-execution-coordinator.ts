...
import type { LLMExecutionConfig } from '@modular-agent/types';
...
export class LLMExecutionCoordinator {
  ...
  private async executeLLMLoop(
    ...
    const {
      contextId, prompt, config, tools,
      abortSignal, eventManager, nodeId
    } = params;

    const {
      profileId, parameters, maxToolCallsPerRequest,
      enableTokenTracking, tokenWarningThreshold, tokenLimit
    } = config;
    ...
    if (tools && tools.length > 0) {
      availableToolSchemas = prepareToolSchemasFromTools(tools);
    }
