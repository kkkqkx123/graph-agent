...
export type ErrorParams<T extends BaseEvent & { error: any }> = Omit<BuildParams<T>, 'error'> & { error: Error };
 ...
 */
export const createBuilder = <T extends BaseEvent>(type: T['type']) =>
 ...
 * @returns Event builder function
 */
export const createBuilderWithOptionalContext = <T extends BaseEvent>(type: T['type']) =>
  (params: BuildParamsWithOptionalContext<T>): T => ({ type, timestamp: now(), ...params } as T);
 ...
 * @returns Event builder function
 */
export const createErrorBuilder = <T extends BaseEvent & { error: any }>(type: T['type']) =>
 ...
 * @returns Event builder function
 */
export const createStringErrorBuilder = <T extends BaseEvent & { error: string }>(type: T['type']) =>
