...
class TriggerTemplateRegistry {
  ...
  private validateTemplate(template: TriggerTemplate): void {
    ...
    // 验证动作类型是否有效
    const validActionTypes = ['start_dynamic_child', 'stop_thread', 'pause_thread', 'resume_thread', 'skip_node', 'set_variable', 'send_notification', 'custom', 'execute_triggered_subgraph', 'execute_script', 'apply_message_operation'];
