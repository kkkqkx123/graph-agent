...
import { generateId, now, getThreadInterruptedException } from '@modular-agent/common-utils';
...
describe('中断功能完整调用链集成测试', () => {
  ...
  describe('暂停（PAUSE）场景完整调用链', () => {
    it('应该验证完整的暂停调用链：触发暂停 → 检测中断 → 抛出异常 → 顶层处理', async () => {
      ...
      expect((abortReason as ThreadInterruptedException).interruptionType).toBe('PAUSE');
  ...
  describe('停止（STOP）场景完整调用链', () => {
    it('应该验证完整的停止调用链：触发停止 → 检测中断 → 抛出异常 → 顶层处理', async () => {
      ...
      const abortReason = getThreadInterruptedException(threadContext.getAbortSignal());
      expect(abortReason).toBeDefined();
      expect(abortReason).toBeInstanceOf(ThreadInterruptedException);
      expect((abortReason as ThreadInterruptedException).interruptionType).toBe('STOP');
      expect((abortReason as ThreadInterruptedException).threadId).toBe(threadContext.getThreadId());
  ...
  describe('恢复（RESUME）场景完整调用链', () => {
    it('应该验证完整的恢复调用链：触发恢复 → 重置中断状态 → 继续执行', async () => {
      ...
      expect(getThreadInterruptedException(threadContext.getAbortSignal())).toBeUndefined();
  ...
  describe('深度中断（AbortSignal）完整调用链', () => {
    ...
    it('应该验证 AbortSignal 事件监听器被正确触发', (done) => {
      ...
      abortSignal.addEventListener('abort', (event) => {
        ...
        expect(getThreadInterruptedException(abortSignal)).toBeInstanceOf(ThreadInterruptedException);
        done();
      });
