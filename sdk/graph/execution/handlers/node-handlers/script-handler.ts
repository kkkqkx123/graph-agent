 ...
 */
export async function scriptHandler(thread: Thread, node: Node, context?: any): Promise<any> {
  const config = node.config as ScriptNodeConfig;

  try {
    // 使用脚本服务执行脚本
    const container = getContainer();
    const scriptService = container.get(Identifiers.ScriptService);
