# Checkpoint与Layertwine集成分析报告

> 生成日期：2026-06-18  
> 分析范围：crates/layertwine与packages/types/checkpoint、sdk/core/checkpoint、sdk/agent/checkpoint集成关系
...
| 模块 | 技术栈 | 责任 | 生命周期 |
|-----|------|------|---------|
| **Layertwine** | Rust | 文件版本控制、checkpoint存储、分支管理 | 独立服务/嵌入式进程 |
| **LayertwineExecutor** | TypeScript | gRPC客户端、进程管理、服务连接 | 随应用生命周期 |
| **CheckpointCoordinator** | TypeScript | Agent/Graph状态快照、delta优化 | 执行过程中 |

---

## 二、Rust端Layertwine Checkpoint实现

### 2.1 核心概念

**Checkpoint** (`crates/layertwine/src/checkpoint/checkpoint.rs`)
...
**CheckpointRepo** (`crates/layertwine/src/checkpoint/repo.rs`)
...
### 4.1 LayertwineExecutor gRPC接口

**gRPC方法映射** (`sdk/services/executors/remote/implementations/layertwine/types.ts`)

| gRPC方法 | 请求 | 响应 | Layertwine功能 |
|---------|------|------|-----------|
| **Commit** | LayertwineCommitRequest | LayertwineCommitResponse | 创建checkpoint |
| **AgentEdit** | LayertwineAgentEditRequest | LayertwineAgentEditResponse | Agent编辑文件 |
| **AgentSubmit** | LayertwineAgentSubmitRequest | LayertwineAgentSubmitResponse | Agent提交checkpoint |
| **Log** | LayertwineLogRequest | LayertwineLogResponse | 查询checkpoint历史 |
| **BranchList** | - | LayertwineBranchListResponse | 列出分支 |
| **Approve** | LayertwineApproveRequest | LayertwineApproveResponse | 批准变更 |
...
**LayertwineExecutorConfig** 支持两种模式：
...
**实现细节**：
- **Embedded**: LayertwineProcessManager启动子进程，等待gRPC服务就绪
- **Remote**: 直接连接到已部署的Layertwine服务
...
│  - 调用saveCheckpoint()回调                      │
├─────────────────────────────────────────────────┤
│ LayertwineExecutor (Services layer)                │
│  - gRPC client wrapper                          │
│  - 进程管理（embedded模式）                      │
│  - 连接池                                        │
├─────────────────────────────────────────────────┤
...
| 功能 | TypeScript Checkpoint | Rust Layertwine Checkpoint | 重合度 |
|-----|---------------------|----------------------|-------|
...
#### Rust Layertwine Checkpoint (File-centric)
```
目标：版本控制文件状态
...
TypeScript Coordinator 的 saveCheckpoint 回调：
  - 何时被调用？
  - 是否通过 LayertwineExecutor.commit() 写入Rust？
  - 还是本地数据库？
...
interface CheckpointLifecycle {
  ...
  // Rust端：提交到Layertwine
  submitToLayertwine(layertwineExecutor, checkpoint): Promise<checkpointId>;
  
  // 同步元数据
  syncMetadata(tsCheckpointId, layertwineCheckpointId);
}
```
...
interface CheckpointIdMapping {
  ...
  layertwineCheckpointId: string; // Rust content hash
  timestamp: number;
  metadata: {
    agentId: string;
    iteration: number;
    layertwineBranch: string;
  }
}
```
...
创建顶层接口：ICheckpointSnapshot
  ...
  └─ FileSnapshot (Layertwine baseline_snapshots)

CheckpointCoordinator 不再直接处理特定类型
而是统一序列化为 ICheckpointSnapshot
然后通过 LayertwineExecutor.commit() 持久化
```
...
TypeScript 层：状态管理（什么时候保存）
  ↓ 不涉及存储细节
Layertwine 层：版本管理（如何存储、分支、合并）
  ↓ 提供统一的持久化API

中间层（适配器）：格式转换
  TypeScript Checkpoint → Layertwine Checkpoint
```
...
考虑创建 CheckpointStore 抽象：
  - 自动将 TypeScript checkpoint 写入 Layertwine
  - 统一查询接口
  - 自动处理ID映射和元数据同步
...
### Rust Layertwine Checkpoint (crates/layertwine)
```
src/checkpoint/
├── mod.rs           # 模块导出
...
### LayertwineExecutor (sdk/services)
```
sdk/services/executors/remote/
├── BaseRemoteExecutor.ts
├── types.ts
└── implementations/layertwine/
    ├── LayertwineExecutor.ts           # 核心gRPC客户端
    ├── layertwine-process.ts           # 子进程管理
    ├── types.ts                      # gRPC类型定义
    └── index.ts
```
...
1. **Layertwine集成方案**：docs/plan/layertwine-integration-plan.md
2. **Rust Checkpoint实现**：crates/layertwine/AGENTS.md
...
| **Branch** | 版本控制中的平行开发线（Layertwine专有） |
| **DAG** | 有向无环图，表示checkpoint的继承关系 |
| **Content-addressed** | 根据内容hash生成ID（Layertwine用） |
| **LayertwineExecutor** | TypeScript中的Layertwine gRPC客户端 |
