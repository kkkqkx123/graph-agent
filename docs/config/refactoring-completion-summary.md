...
### 代码重复清理
根据用户反馈，我们进一步清理了代码中的重复和冗余：

1. **简化类型定义** - 移除了types.ts中不再使用的接口（如IModuleRule, IModuleLoader等）
2. **统一导入方式** - 将schemas/index.ts中的require改为ES6导入，提高模块化程度
3. **确保一致性** - 保证类型定义与实际实现的一致性
...
6. **清理了代码重复** - 移除了冗余的类型定义和不一致的导入方式

重构后的框架更加简洁、高效，同时保持了功能的完整性，为项目的长期发展奠定了坚实的基础。通过进一步的代码清理，我们确保了架构的清晰度和代码的一致性。