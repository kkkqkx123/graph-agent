...
describe('SqliteCheckpointStorage', () => {
  ...
  describe('list', () => {
    beforeEach(async () => {
      await storage.save('cp-1', new Uint8Array([1]), createMetadata({ threadId: 'thread-1', timestamp: 1000 }));
      await storage.save('cp-2', new Uint8Array([2]), createMetadata({ threadId: 'thread-1', timestamp: 2000 }));
      await storage.save('cp-3', new Uint8Array([3]), createMetadata({ threadId: 'thread-2', timestamp: 3000 }));
    });
