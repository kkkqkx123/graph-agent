...
import type { WorkflowStorageMetadata } from '@modular-agent/types';
...
describe('JsonWorkflowStorage', () => {
  ...
  describe('initialize', () => {
    it('should create versions directory', async () => {
  ...
  describe('list', () => {
    beforeEach(async () => {
      ...
      await storage.save('workflow-2', new Uint8Array([2]), createMetadata({
      ...
      await storage.save('workflow-3', new Uint8Array([3]), createMetadata({
  ...
  describe('updateWorkflowMetadata', () => {
    it('should update workflow metadata', async () => {
      await storage.save('workflow-1', new Uint8Array([1, 2, 3]), createMetadata({
        name: 'Old Name',
    ...
    it('should throw for non-existent workflow', async () => {
      await expect(storage.updateWorkflowMetadata('non-existent', { name: 'test' })).rejects.toThrow();
    });
  });
