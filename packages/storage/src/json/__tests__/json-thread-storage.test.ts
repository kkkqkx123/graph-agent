...
import type { ThreadStorageMetadata, ThreadStatus } from '@modular-agent/types';
...
describe('JsonThreadStorage', () => {
  ...
  describe('list', () => {
    beforeEach(async () => {
      ...
      await storage.save('thread-3', new Uint8Array([3]), createMetadata({
        threadId: 'thread-3',
  ...
  describe('updateThreadStatus', () => {
    it('should update thread status', async () => {
      await storage.save('thread-1', new Uint8Array([1, 2, 3]), createMetadata({ status: 'RUNNING' }));
