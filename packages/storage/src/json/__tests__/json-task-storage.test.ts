...
import type { TaskStorageMetadata, TaskStatus } from '@modular-agent/types';
...
describe('JsonTaskStorage', () => {
  ...
  describe('list', () => {
    beforeEach(async () => {
      ...
      await storage.save('task-3', new Uint8Array([3]), createMetadata({
        taskId: 'task-3',
        threadId: 'thread-2',
        workflowId: 'workflow-1',
        status: 'COMPLETED',
  ...
  describe('getTaskStats', () => {
    beforeEach(async () => {
      ...
      await storage.save('task-4', new Uint8Array([4]), createMetadata({
        workflowId: 'workflow-1',
        status: 'QUEUED',
        submitTime: 4000
      }));
      await storage.save('task-5', new Uint8Array([5]), createMetadata({
        workflowId: 'workflow-1',
        status: 'TIMEOUT',
        submitTime: 5000
      }));
  ...
  describe('cleanupTasks', () => {
    beforeEach(async () => {
      const now = Date.now();
      // Old completed task
      await storage.save('task-old', new Uint8Array([1]), createMetadata({
        status: 'COMPLETED',
        completeTime: now - 10000
      }));
      // Recent completed task
      await storage.save('task-recent', new Uint8Array([2]), createMetadata({
        status: 'COMPLETED',
        completeTime: now - 100
      }));
      // Running task (should not be cleaned)
      await storage.save('task-running', new Uint8Array([3]), createMetadata({
        status: 'RUNNING'
      }));
      // Old failed task
      await storage.save('task-failed', new Uint8Array([4]), createMetadata({
        status: 'FAILED',
        completeTime: now - 10000
      }));
    });
