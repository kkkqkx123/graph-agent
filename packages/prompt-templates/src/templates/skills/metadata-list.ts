 ...
 */
export function generateSkillMetadataListPrompt(skills: Array<{
  name: string;
  description: string;
  version?: string;
}>): string {
  ...
  const skillList = skills
    .map(skill => {
      let item = SKILL_METADATA_ITEM_TEMPLATE
        .replace('{{skillName}}', skill.name)
        .replace('{{skillDescription}}', skill.description);

      if (skill.version) {
        item = item.replace('{{version}}', SKILL_VERSION_TEMPLATE.replace('{{version}}', skill.version));
      } else {
