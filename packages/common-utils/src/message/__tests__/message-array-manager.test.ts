...
import { MessageArrayManagerImpl } from '../message-array-manager-impl';
...
describe('MessageArrayManager', () => {
  ...
  describe('初始化', () => {
    it('应该正确初始化空消息数组', () => {
      const manager = new MessageArrayManagerImpl();
    ...
    it('应该正确初始化带初始消息的数组', () => {
      const manager = new MessageArrayManagerImpl(initialMessages);
      const state = manager.getState();
  ...
  describe('APPEND 操作', () => {
    it('应该正确追加消息（不创建新批次）', () => {
      ...
      expect(result.state.messages[3]).toEqual(newMessages[0]);
      expect(result.affectedBatchIndex).toBe(0);
      expect(result.state.batchSnapshots).toHaveLength(0);
      expect(result.state.currentBatchIndex).toBe(0);
    });
    ...
    it('应该支持批量追加', () => {
      ...
      expect(result.state.messages).toHaveLength(5);
      expect(result.state.messages[3]).toEqual(newMessages[0]);
      expect(result.state.messages[4]).toEqual(newMessages[1]);
    });
  });
  ...
  describe('INSERT 操作', () => {
    it('应该正确在中间插入消息（创建新批次）', () => {
      ...
      expect(result.state.batchSnapshots).toHaveLength(1);
      expect(result.state.currentBatchIndex).toBe(1);
    });
    ...
    it('应该在开头插入消息', () => {
      ...
      expect(result.state.messages[0]).toEqual(newMessage);
      expect(result.state.messages[1]).toEqual(initialMessages[0]);
    });
    ...
    it('应该在末尾插入消息', () => {
      ...
      expect(result.state.messages[3]).toEqual(newMessage);
    });
  ...
  describe('REPLACE 操作', () => {
    it('应该正确替换消息（创建新批次）', () => {
      ...
      expect(result.state.messages[1]).toEqual(newMessage);
      expect(result.state.messages[0]).toEqual(initialMessages[0]);
      expect(result.affectedBatchIndex).toBe(1);
      expect(result.state.batchSnapshots).toHaveLength(1);
    });
  ...
  describe('TRUNCATE 操作', () => {
    it('应该正确保留前N条消息', () => {
      ...
      expect(result.state.messages[0]).toEqual(initialMessages[0]);
      expect(result.state.messages[1]).toEqual(initialMessages[1]);
    });
    ...
    it('应该正确保留后N条消息', () => {
      ...
      expect(result.state.messages).toHaveLength(2);
      expect(result.state.messages[0]).toEqual(initialMessages[1]);
      expect(result.state.messages[1]).toEqual(initialMessages[2]);
    });
  ...
  describe('FILTER 操作', () => {
    it('应该正确按角色过滤', () => {
      ...
      expect(result.state.messages).toHaveLength(2);
      expect(result.state.messages.every(msg => msg.role !== 'system')).toBe(true);
    });
    ...
    it('应该正确按内容关键词过滤（包含）', () => {
      ...
      expect(result.state.messages).toHaveLength(1);
      expect(result.state.messages[0].content).toContain('Hello');
    });
    ...
    it('应该正确按内容关键词过滤（排除）', () => {
      ...
      expect(result.state.messages).toHaveLength(2);
      expect(result.state.messages.every(msg => !msg.content.includes('Hello'))).toBe(true);
    });
