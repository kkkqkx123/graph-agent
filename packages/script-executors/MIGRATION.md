...
4. **简化的 ScriptService**: 职责更加单一，专注于脚本管理和执行器协调
5. **重命名**: 所有 "Code" 相关的命名已改为 "Script"，提高可读性
...
scriptService.registerExecutor('SHELL', new MyShellExecutor());
```
...
scriptService.registerExecutor('SHELL', shellExecutor);
```
...
### ScriptService API 保持不变

`ScriptService` 的公共 API 保持不变，现有代码无需修改：

```typescript
// 这些 API 保持不变
scriptService.registerScript(script);
scriptService.execute(scriptName, options);
scriptService.executeBatch(executions);
```
...
```typescript
// 注册方式保持不变
scriptService.registerExecutor('SHELL', executor);
```
...
describe('ScriptService', () => {
  it('should execute shell script', async () => {
    ...
    const result = await scriptService.execute('test');
...
A: 不会。`ScriptService.execute()` 方法仍然接受 `Partial<ScriptExecutionOptions>`，并在内部合并选项。

### Q: CodeService 改名为 ScriptService 了吗？

A: 是的。为了提高可读性，所有 "Code" 相关的命名都已改为 "Script"：
- `CodeService` → `ScriptService`
- `CodeExecutionError` → `ScriptExecutionError`
- `CodeHandler` → `ScriptHandler`
- `CODE_NODE` → `SCRIPT_NODE`
- `CodeNodeConfig` → `ScriptNodeConfig`
- `validateCodeNode` → `validateScriptNode`

这些更改使命名更加准确和一致，因为实际执行的是脚本而不是通用的"代码"。
