...
export class LLMResponseRepository
  ...
  protected override toDomain(model: LLMResponseModel): LLMResponse {
    try {
      ...
      const errorMessage = `LLMResponse模型转换失败: ${error instanceof Error ? error.message : String(error)}`;
  ...
  protected override toModel(entity: LLMResponse): LLMResponseModel {
    try {
      ...
      const errorMessage = `LLMResponse实体转换失败: ${error instanceof Error ? error.message : String(error)}`;
      const customError = new Error(errorMessage);
      (customError as any).code = 'MAPPING_ERROR';
      (customError as any).context = { entityId: entity.responseId.value, operation: 'toModel' };
      throw customError;
    }
  }
