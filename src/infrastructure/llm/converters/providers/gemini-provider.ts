...
export class GeminiProvider extends BaseProvider {
  ...
  private convertMessage(
    ...
    if (toolCalls && toolCalls.length > 0) {
      providerMessage['function_calls'] = this.convertToolCalls(toolCalls, context);
    }
  ...
  private convertSystemMessage(
    ...
    return {
      role: 'user',
      parts: this.processContent(message.getContent(), context)
    };
  }
  ...
  private getRoleMapping(role: string): string {
    const roleMapping: Record<string, string> = {
      ...
      'assistant': 'model',
      'tool': 'function',
      'function': 'function' // 处理FUNCTION角色
    };
