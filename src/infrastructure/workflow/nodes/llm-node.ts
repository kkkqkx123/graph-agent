...
export class LLMNode extends Node {
  constructor(
    ...
    super(
      id,
      NodeType.llm(NodeContextTypeValue.LLM_CONTEXT),
      name,
      description,
      position
    );
  ...
  async execute(context: WorkflowExecutionContext): Promise<NodeExecutionResult> {
    ...
    try {
      ...
      // 通过包装器服务调用LLM
      const llmResponse = await wrapperService.generateResponse(this.wrapperName, llmRequest);
  ...
  validate(): ValidationResult {
    ...
    } else if (!this.prompt.type || (this.prompt.type !== 'direct' && this.prompt.type !== 'template')) {
    ...
    if (this.systemPrompt) {
      ...
      } else if (!this.systemPrompt.type || (this.systemPrompt.type !== 'direct' && this.systemPrompt.type !== 'template')) {
      ...
      } else if (this.systemPrompt.type === 'template' && (!this.systemPrompt.category || !this.systemPrompt.name)) {
        errors.push('systemPrompt.category和systemPrompt.name在template类型下是必需的');
      }
    }

    if (this.temperature !== undefined && (typeof this.temperature !== 'number' || this.temperature < 0 || this.temperature > 2)) {
      errors.push('temperature必须是0-2之间的数字');
    }

    if (this.maxTokens !== undefined && (typeof this.maxTokens !== 'number' || this.maxTokens <= 0)) {
