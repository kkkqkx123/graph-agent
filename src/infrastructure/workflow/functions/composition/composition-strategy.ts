...
export class AndCompositionStrategy implements CompositionStrategy {
  async execute(functions: BaseWorkflowFunction[], context: any, config: any): Promise<boolean> {
    for (const func of functions) {
  ...
  validate(functions: BaseWorkflowFunction[]): ValidationResult {
    ...
    // 验证所有函数都返回boolean类型
    for (const func of functions) {
      if (func.getReturnType() !== 'boolean') {
        errors.push(`函数 ${func.name} 必须返回boolean类型`);
      }
    }
