...
export class ParallelStrategy extends ExecutionStrategy {
  async execute(context: ExecutionContext, workflowExecutor: WorkflowExecutor): Promise<any> {
    ...
    await this.updateExecutionQueue(workflow, executedNodes, executionQueue, context, workflowExecutor);
  ...
  async executeWithDependencies(context: ExecutionContext, workflowExecutor: WorkflowExecutor): Promise<any> {
    const workflow = context.getWorkflow();
  ...
  async executeWithConcurrencyLimit(
    ...
    while (executionQueue.length > 0 || runningTasks.size > 0) {
      ...
      while (executionQueue.length > 0 && runningTasks.size < concurrencyLimit) {
        ...
        const task = this.executeNode(nodeId, context, workflowExecutor)
          .then(result => {
            executedNodes.add(nodeId);
            nodeResults.set(nodeId, result);
