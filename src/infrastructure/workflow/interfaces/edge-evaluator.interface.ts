...
export interface IEdgeEvaluator {
   ...
   * @returns 是否可以执行
   */
  evaluate(edge: EdgeValueObject, context: any): Promise<boolean>;
   ...
   * @returns 验证结果
   */
  validate(edge: EdgeValueObject): Promise<{ valid: boolean; errors: string[] }>;
   ...
   * @returns 变量名列表
   */
  extractVariables(edge: EdgeValueObject): string[];
