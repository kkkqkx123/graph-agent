 ...
 * 状态快照未找到异常
 */
export class StateSnapshotNotFoundError extends StateError {
  constructor(snapshotId: string) {
    super(`状态快照未找到: ${snapshotId}`, 'STATE_SNAPSHOT_NOT_FOUND', { snapshotId });
  }
}
 ...
 * 状态快照创建错误异常
 */
export class StateSnapshotCreationError extends StateError {
  constructor(threadId: string, reason: string) {
    super(`状态快照创建错误: ${threadId} - ${reason}`, 'STATE_SNAPSHOT_CREATION_ERROR', {
      threadId,
      reason,
    });
  }
 ...
 * 状态快照恢复错误异常
 */
export class StateSnapshotRestoreError extends StateError {
  constructor(snapshotId: string, reason: string) {
    super(`状态快照恢复错误: ${snapshotId} - ${reason}`, 'STATE_SNAPSHOT_RESTORE_ERROR', {
      snapshotId,
      reason,
    });
  }
 ...
 * 状态快照删除错误异常
 */
export class StateSnapshotDeletionError extends StateError {
  constructor(snapshotId: string, reason: string) {
    super(`状态快照删除错误: ${snapshotId} - ${reason}`, 'STATE_SNAPSHOT_DELETION_ERROR', {
      snapshotId,
      reason,
    });
  }
 ...
 * 状态快照范围不匹配异常
 */
export class StateSnapshotScopeMismatchError extends StateError {
  constructor(snapshotId: string, expectedScope: string, actualScope: string) {
    super(
      `状态快照范围不匹配: ${snapshotId} - 期望: ${expectedScope}, 实际: ${actualScope}`,
      'STATE_SNAPSHOT_SCOPE_MISMATCH',
      { snapshotId, expectedScope, actualScope }
    );
  }
}
 ...
 * 状态快照目标不匹配异常
 */
export class StateSnapshotTargetMismatchError extends StateError {
  constructor(snapshotId: string, expectedTargetId: string, actualTargetId: string) {
    super(
      `状态快照目标不匹配: ${snapshotId} - 期望: ${expectedTargetId}, 实际: ${actualTargetId}`,
      'STATE_SNAPSHOT_TARGET_MISMATCH',
      { snapshotId, expectedTargetId, actualTargetId }
    );
  }
}
