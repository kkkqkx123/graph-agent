...
export interface ICheckpointRepository extends Repository<Checkpoint> {
   ...
   * @returns 检查点列表
   */
  findByStatus(status: CheckpointStatus): Promise<Checkpoint[]>;
   ...
   * @returns 检查点列表
   */
  findByType(type: CheckpointType): Promise<Checkpoint[]>;
   ...
   * @returns 检查点历史列表
   */
  getThreadHistory(threadId: ID, limit?: number, offset?: number): Promise<Checkpoint[]>;
   ...
   */
  getLatest(threadId: ID): Promise<Checkpoint | null>;
